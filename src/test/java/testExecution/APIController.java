...
public class APIController {
	...
	// read the data from congigProperties
		public static String reademailproperties(String skey) {
			String svalue = null;
			Properties properties = new Properties();
			try {
				FileInputStream fis = new FileInputStream(sconfigemail);
				properties.load(fis);
				svalue = properties.getProperty(skey);
	...
	public static RequestSpecification buildReq(String str_Authvalue) {
		...
		RequestSpecification req = new RequestSpecBuilder().setBaseUri("http://konnectautomation.kirusa.com:8080/api/v1/Accounts").addHeader("Content-Type","application/json").addHeader("Authorization",str_Authvalue).build();
		return req;
